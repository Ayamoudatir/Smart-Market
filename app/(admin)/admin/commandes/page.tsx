          ...
          return (
            <button key={t.value} onClick={() => setFilter(t.value)}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-sm font-medium transition ${
                filter === t.value ? 'bg-green-600 text-white' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50'
              }`}>
              {t.label}
          ...
          <table className="w-full min-w-[700px]">
            <thead>
              <tr className="border-b border-gray-100">
                <th className="text-left text-xs font-semibold text-gray-400 uppercase tracking-wide px-5 py-3">Commande</th>
                ...
                <th className="text-left text-xs font-semibold text-gray-400 uppercase tracking-wide px-5 py-3">Changer</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map(o => {
                ...
                return (
                  <Fragment key={o.id}>
                    <tr
                      className={`border-b border-gray-50 transition cursor-pointer ${isOpen ? 'bg-green-50/30' : 'hover:bg-gray-50'}`}
                      onClick={() => setExpanded(isOpen ? null : o.id)}
                    >
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-2">
                          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className={`text-gray-400 transition-transform ${isOpen ? 'rotate-90' : ''}`}><polyline points="9 18 15 12 9 6"/></svg>
                      ...
                      <td className="px-5 py-4">
                        ...
                        <p className="text-xs text-gray-400 truncate max-w-[160px]">{o.clientAddress}</p>
                      </td>
                      ...
                      <td className="px-5 py-4" onClick={e => e.stopPropagation()}>
                        {saving === o.id ? (
                          ...
                          <select
                            ...
                            className="border border-gray-200 rounded-xl px-3 py-1.5 text-xs text-gray-700 outline-none focus:border-green-500 bg-white"
                          >
                      ...
                      <tr className="border-b border-gray-100 bg-gray-50/50">
                        <td colSpan={6} className="px-10 py-4">
                          <div className="space-y-1.5">
                            ...
                            {o.clientPhone && (
                              <p className="text-xs text-gray-400 pt-2">📞 {o.clientPhone}</p>
                            )}
                          </div>
                        </td>
                      </tr>
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={6} className="px-5 py-10 text-center text-sm text-gray-400">Aucune commande trouvée</td></tr>
              )}
            </tbody>
          </table>
        </div>
