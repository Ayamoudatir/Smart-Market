          ...
          <thead>
            <tr className="border-b border-green-100">
              {['Utilisateur', 'Action', 'Cible', 'Date'].map(h => (
                <th key={h} className="text-left text-xs font-semibold text-green-800/40 uppercase tracking-wide px-5 py-3">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {logs.map(l => (
              <tr key={l.id} className="border-b border-gray-50 hover:bg-green-50/50 transition">
                ...
                <td className="px-5 py-3.5 text-xs text-green-800/40">{l.timestamp ? new Date((l.timestamp as unknown as { seconds: number }).seconds * 1000).toLocaleString('fr-MA') : '—'}</td>
              </tr>
            ))}
            {logs.length === 0 && <tr><td colSpan={4} className="px-5 py-10 text-center text-sm text-green-800/40">Aucun log</td></tr>}
          </tbody>
        </table></div>
      </div>
    </div>
