  ...
  return (
    <div>
      ...
      <div className="space-y-3">
        {filtered.length === 0 && <div className="bg-white/80 backdrop-blur-sm rounded-2xl border border-green-100 p-10 text-center text-sm text-green-800/40">Aucune commande</div>}
        {filtered.map(o => (
          <Link key={o.id} href={`/preparateur/commandes/${o.id}`}
            className="bg-white/80 backdrop-blur-sm rounded-2xl border border-green-100 p-5 flex items-center justify-between hover:border-green-300 transition block">
            <div>
              ...
              <p className="text-xs text-green-800/40 mt-1">{o.items.length} article{o.items.length > 1 ? 's' : ''}</p>
            </div>
