    ...
    <div>
      ...
      <div className="bg-white/80 backdrop-blur-sm rounded-2xl border border-green-100 overflow-hidden">
        <div className="overflow-x-auto"><table className="w-full min-w-[600px]">
          ...
          <tbody>
            {movements.map(m => (
              <tr key={m.id} className="border-b border-gray-50 hover:bg-green-50/50 transition">
                ...
                <td className="px-5 py-3.5 text-xs text-green-800/40">{m.date ? new Date((m.date as unknown as { seconds: number }).seconds * 1000).toLocaleDateString('fr-MA') : '-'}</td>
              </tr>
            ))}
            {movements.length === 0 && (
              <tr><td colSpan={6} className="px-5 py-10 text-center text-sm text-green-800/40">Aucun mouvement enregistré</td></tr>
            )}
          </tbody>
        </table></div>
      </div>
