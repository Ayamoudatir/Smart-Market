...
export default function AjouterProduit() {
  ...
  function set(k: keyof typeof form) { return (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => setForm(prev => ({ ...prev, [k]: e.target.value })) }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      await addProduct({
        name: form.name,
        categoryId: form.categoryName.toLowerCase(),
    ...
    <div className="max-w-2xl">
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-gray-200 p-6 space-y-5">
        ...
        <div className="flex gap-3 pt-2">
          <button type="button" onClick={() => router.back()} className="flex-1 border border-gray-300 text-gray-700 font-medium py-2.5 rounded-xl hover:bg-gray-50 transition text-sm">Annuler</button>
          <button type="submit" disabled={loading} className="flex-1 bg-green-600 hover:bg-green-700 disabled:opacity-60 text-white font-semibold py-2.5 rounded-xl transition text-sm">{loading ? 'Enregistrement…' : 'Enregistrer'}</button>
        </div>
      </form>
    </div>
  )
