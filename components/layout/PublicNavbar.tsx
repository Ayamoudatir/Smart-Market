...
export default function PublicNavbar({ search = '', onSearch, placeholder = 'Rechercher un produit…' }: Props) {
  ...
  return (
    <>
      ...
      {/* Navbar */}
      <header className="relative z-50 px-6 md:px-14 py-4 flex items-center gap-4">
        <Link href="/" className="shrink-0">
          <Image src="/assets/kenzi_logo.png" alt="Kenzi Market" width={100} height={100} className="object-contain drop-shadow-lg" />
        </Link>

        <div className="flex-1 max-w-xl mx-2">
          <div className="flex items-center bg-white/70 backdrop-blur-md rounded-xl overflow-hidden border border-gray-200 focus-within:border-[#1a5c2a] transition">
        ...
        <div className="flex items-center gap-2 ml-auto shrink-0">
          ...
          <Link href={user ? '/compte' : '/login'}
            className="flex flex-col items-center gap-0.5 px-3 py-1.5 rounded-xl hover:bg-black/5 transition">
            {initials ? (
              <div className="w-7 h-7 rounded-full bg-[#1a5c2a] flex items-center justify-center text-white text-[10px] font-black">
                {initials}
              </div>
            ) : (
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-gray-700"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
            )}
            <span className="text-xs text-gray-600">{user ? (displayName.split(' ')[0] || 'Mon compte') : 'Mon compte'}</span>
          </Link>

          {/* Mon panier */}
          <Link href="/panier" className="relative flex flex-col items-center gap-0.5 px-3 py-1.5 rounded-xl hover:bg-black/5 transition">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-gray-700"><circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/><path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"/></svg>
            <span className="text-xs text-gray-600">Mon panier</span>
            {totalItems > 0 && (
